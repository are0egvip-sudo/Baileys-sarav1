            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        newsletterJid: "120363408691902765@newsletter",
                        serverMessageId: 0,
                        newsletterName: "𝚂𝙰𝚁𝙰|⃟ ♡︎|𝙱𝙾𝚃メ ⃢𝚅1",
                        contentType: "UPDATE",
                    }
                }
            ]
...
const generateWAMessageContent = async (message, options) => {
    ...
    // ✅ إضافة دعم stickerPack
    else if ('stickerPack' in message) {
        const pack = message.stickerPack;
        const stickers = pack.stickers || [];
        if (!stickers.length) throw new boom_1.Boom('لا توجد ملصقات', { statusCode: 400 });
        
        // إرسال معلومات الحزمة كنص مع صورة مصغرة
        m = {
            extendedTextMessage: {
                text: `📦 *${pack.name || 'حزمة ملصقات'}*\n👤 ${pack.publisher || 'غير معروف'}\n📊 ${stickers.length} ملصق`,
                contextInfo: {
                    externalAdReply: {
                        title: pack.name || 'Sticker Pack',
                        body: `${stickers.length} ملصق`,
                        thumbnailUrl: pack.cover?.url || 'https://telegra.ph/file/1d6a0d5e2a9e8c6d8e5f0.jpg',
                        mediaType: 1
                    }
                }
            }
        ...
        // تخزين الملصقات لإرسالها بعد الرسالة
        if (!global._pendingStickerPack) global._pendingStickerPack = [];
        global._pendingStickerPack.push({ jid: options.userJid, stickers: stickers });
    }
    else {
